...
if [ "${DT_DEPLOYED_ONBOARD}" = "1" ] && [ "${netmode}" = "HOST" ]; then
    export DTPS_BASE_SWITCHBOARD_1="http://localhost:11911/"
else
    # prefer IP address over mDNS
    if [ -n "${VEHICLE_IP}" ]; then
        export DTPS_BASE_SWITCHBOARD_1="http://${VEHICLE_IP}:11911/"
    else
        export DTPS_BASE_SWITCHBOARD_1="http://${VEHICLE_NAME}.local:11911/"
    fi
fi
