    ...
    for src in "${PROJECTS_LOCATIONS[@]}"; do
        ...
        for d in $(find "${src}" -mindepth 1 -maxdepth 1 -type d -printf "%T@ %p\n" | sort -n | cut -d " " -f 2); do
            ...
            if [ -d "${PROJECT_ENTRYPOINT_DIR}" ]; then
                ...
                for f in $(find "${PROJECT_ENTRYPOINT_DIR}" -mindepth 1 -maxdepth 1 -type f -name "*.sh"); do
                    debug "  > Sourcing ${f}"
                    source ${f}
                    debug "  < Sourced ${f}"
                done
            ...
            if [ -d "${PROJECT_LIBRARIES_DIR}" ]; then
                ...
                for lib in $(find "${PROJECT_LIBRARIES_DIR}" -mindepth 1 -maxdepth 1 -type d); do
                    ...
                    if [ -f "${LIBRARY_SETUP_PY}" ] || [ -f "${LIBRARY_PYPROJECT_TOML}" ]; then
                        ...
                        # ---
                        if [[ ( -d "${lib}/dtproject" || -f "${lib}/.dtproject" ) && -d "${lib}/src" ]]; then
                            # we recognize the library as a dtproject, use the PYTHONPATH trick
                            info "  > Adding ${lib}/src to PYTHONPATH"
...
if [ ${#DT_LAUNCHER} -gt 0 ] && [ "$1" == "--" ]; then
    ...
    if [[ "${DT_SUPERUSER:-0}" == "1" ]]; then
        ...
        exec sudo --set-home --preserve-env --user ${DT_USER_NAME} LD_LIBRARY_PATH=$LD_LIBRARY_PATH PYTHONPATH=$PYTHONPATH PATH=$PATH bash -c "dt-launcher-$DT_LAUNCHER $*"
    fi
else
    if [[ "${DT_SUPERUSER:-0}" == "1" ]]; then
        exec "$@"
    else
        exec sudo --set-home --preserve-env --user ${DT_USER_NAME} LD_LIBRARY_PATH=$LD_LIBRARY_PATH PYTHONPATH=$PYTHONPATH PATH=$PATH "$@"
    fi
fi
